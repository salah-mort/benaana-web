...
const practices = [
  ["01", "التوقف المؤقت", "دقيقة فاصلة قبل الرد حين يرتفع الضغط داخل الخيمة أو البيت."],
...
const steps = [
  ...
  ["02", "أهدأ", "أستخدم التوقف المؤقت وأؤجل الرد المؤذي.", "تنظيم"],
  ["03", "أقترب", "أسأل وأصغي وأشارك مهمة واحدة قابلة للملاحظة.", "شراكة"],
  ["04", "أنقل", "أشارك أداة نافعة مع أب آخر في الحي.", "استدامة"],
] as const;

const impactStats = [
  ["30", "", "أباً نازحاً", "في مجموعتين تدريبيتين حضوريتين", UsersRound],
  ["06", "", "جلسات تدريبية", "جلسة أسبوعياً لكل مجموعة", CalendarDays],
  ["30", "", "حقيبة أمان عائلة", "مستلزمات طوارئ وإضاءة وأدوات تفاعلية وألعاب", Target],
  ["80", "%", "تطبيقاً مستهدفاً", "لخطة «التوقف المؤقت» وإدارة الضغوط داخل المنازل والخيام", BarChart3],
] as const;

const impactBreakdown = [
  { id: "group-one", label: "المجموعة الأولى", percent: 50, detail: "15 أباً يتدربون على التفريغ النفسي وإدارة الغضب و«التوقف المؤقت»." },
  { id: "group-two", label: "المجموعة الثانية", percent: 50, detail: "15 أباً يتدربون على الوالدية الإيجابية وحماية الأسرة دون عنف." },
] as const;

// المخرجات كما وردت في نموذج المبادرة
const outputs = [
  ["01", "تدريب 30 أباً", "على مهارات التفريغ النفسي وإدارة الغضب والتربية الإيجابية خلال 6 أسابيع.", UsersRound],
  ["02", "30 حقيبة أمان عائلة", "مستلزمات طوارئ وإضاءة وأدوات تفاعلية وألعاب للأطفال.", ShieldCheck],
  ["03", "80% تطبيقاً", "لخطة «التوقف المؤقت» وإدارة الضغوط داخل المنازل والخيام.", Target],
  ["04", "حوار أسري أفضل", "خفض أساليب العنف والإساءة تجاه الأبناء والمراهقين (12–17 سنة).", HeartHandshake],
  ["05", "إحالات آمنة", "إحالة الحالات النفسية والاجتماعية الحادة إلى المسارات المتخصصة.", Waypoints],
] as const;

const workshops = [
  ["01", "الأب تحت الضغط", "فهم ضغوط النزوح وتسمية الحمل النفسي"],
  ["02", "إدارة الغضب والتفريغ", "تقنيات التهدئة و«التوقف المؤقت»"],
  ["03", "الرجولة الإيجابية", "تمييز السلوك التقليدي من الإيجابي"],
  ["04", "الوالدية الإيجابية", "بدائل العقاب والحوار المحترم"],
  ["05", "حماية الأسرة", "حل الخلاف دون عنف أو تهديد"],
  ["06", "التواصل مع المراهقين", "لغة حوار مع الفتيان 12–17 سنة"],
] as const;

// الأنشطة الأسرية المشتركة التي يشارك فيها الأبناء إلى جانب الآباء
const familyActivities = [
  ["شعار عائلتنا", "ورشة «تواصل وتعبير» يرسم فيها كل أب وأبناؤه شعار عائلتهم، ويتفقون على ثلاث قيم يلتزم بها البيت.", "الآباء والأبناء · جلسة لكل مجموعة", Sparkles],
  ["حوار مع المراهقين", "جلسة حوارية موجهة بين الآباء والفتيان (12–17 سنة) لبناء لغة حوار مشتركة وتضييق الفجوة الأسرية.", "الآباء والمراهقون · جلسة لكل مجموعة", MessageCircle],
  ["حقيبة أمان العائلة", "تسليم 30 حقيبة تضم مستلزمات طوارئ وإضاءة وأدوات تفاعلية وألعاباً للأطفال.", "30 عائلة · الأسبوع السابع", ShieldCheck],
  ["معرض «لوحات عائلاتنا»", "معرض بسيط في اللقاء الختامي تُعرض فيه اللوحات والشعارات من عمل العائلات، مع توزيع شهادات المشاركة.", "جميع المشاركين · الحدث الختامي", BookOpen],
] as const;

// بنود الميزانية بإجمالي 4,000 دولار
const budgets = [
  ["ضيافة ومياه للجلسات التدريبية", "180 شخص/جلسة × 2.5$", "450$"],
  ["حقائب أمان منزلية ومواد دعم أسري", "30 حقيبة × 67$", "2,000$"],
  ["أدوات ومواد رسم وتفريغ نفسي", "طقم قرطاسية للورش والمعرض", "250$"],
  ["مكافأة وتسهيل الميسرين النفسيين", "مقطوعية", "800$"],
  ["مطبوعات وشهادات ومعرض ختامي", "طقم واحد", "200$"],
  ["مواصلات وتنقلات ميدانية", "شهران × 150$", "300$"],
] as const;

// خطة التنفيذ على ثمانية أسابيع
const weeks = [
  ["01", "التهيئة والتنسيق", "منسق المبادرة والفريق اللوجستي", ["إعداد المكان واختيار المشاركين", "حصر 30 أباً نازحاً", "تقسيمهم إلى مجموعتين"], "قوائم المشاركين ومساحة آمنة جاهزة"],
  ["02–03", "إدارة الغضب وتفريغ الضغوط", "الميسر النفسي", ["الجلستان الأولى والثانية", "تقنيات التهدئة", "التعامل مع ضغوط النزوح"], "أداة «التوقف المؤقت» قابلة للتطبيق"],
  ["04–05", "الوالدية الإيجابية وحماية الأسرة", "الميسر المجتمعي", ["الجلستان الثالثة والرابعة", "تعزيز التواصل الأسري", "الحد من العقاب القاسي"], "خطة أسرية أولية لكل مشارك"],
  ["06", "الورش الأسرية المشتركة", "الميسرون ومنسق المبادرة", ["ورشة «شعار عائلتنا»", "حوار مع المراهقين", "دمج الأبناء في الحوار"], "شعار وقيم متفق عليها لكل عائلة"],
  ["07", "حقائب الأمان", "مسؤول الشراء واللوجستيات", ["شراء وتجهيز الحقائب", "توزيع 30 حقيبة", "مواد الرسم والتفريغ"], "30 حقيبة أمان عائلة مسلّمة"],
  ["08", "المعرض الختامي والتقييم", "منسق المبادرة وفريق المتابعة", ["معرض لوحات العائلات", "توزيع شهادات المشاركة", "التقييم وإعداد التقارير"], "معرض ختامي وتقرير أثر موثق"],
] as const;

const faqs = [
  ["هل المبادرة للآباء فقط؟", "محور التدخل هو الأب، لكن أثره يستهدف الأسرة كلها. يشارك الأبناء والمراهقون في ورشة «شعار عائلتنا» والجلسات الحوارية والمعرض الختامي، وتصل الأسرة عبر حقيبة أمان العائلة."],
  ["أين تنفذ الأنشطة؟", "في مساحات آمنة ومناسبة داخل أماكن الإيواء أو المجتمع المحلي، وبمجموعتين صغيرتين تراعيان الخصوصية والظروف الأمنية."],
  ["هل هي حملة توعية؟", "لا. هي تدخل ميداني تطبيقي على مدى 8 أسابيع: 6 جلسات تدريبية لمجموعتين، وورش أسرية مشتركة، و30 حقيبة أمان، ومعرض ختامي لأعمال العائلات."],
  ["ماذا يحدث إذا احتاج أب إلى دعم متخصص؟", "تُحفظ الخصوصية، ويجري الميسر إحالة آمنة إلى الخدمات النفسية أو الاجتماعية المتاحة وفق بروتوكول الشريك والمنطقة."],
  ["ما الذي يبقى بعد انتهاء المبادرة؟", "شعار وقيم متفق عليها داخل كل بيت، وأدوات تهدئة مجرَّبة، وحقيبة أمان لكل عائلة، ولوحات معروضة توثق ما أنتجته العائلات بنفسها."],
] as const;
...
const storyScenes = [
  ...
  { number: "05", label: "المشهد الخامس", title: "ست جلسات من الفهم إلى التثبيت", intro: "ست جلسات للآباء تتدرج من فهم الضغط وإدارة الغضب إلى الوالدية الإيجابية والحماية دون عنف، ثم تنضم الأسرة في ورشة «شعار عائلتنا» وحوار المراهقين ومعرض ختامي لأعمال العائلات.", prompt: "افتح المسار التدريبي.", page: 4, image: mentorshipImage, caption: "التدريب · ست جلسات حضورية", tone: "terracotta" },
  ...
  { number: "09", label: "المشهد التاسع", title: "تمويل واضح لنتيجة واضحة", intro: "أكبر بند هو 30 حقيبة أمان عائلة (2,000$)، يليه تيسير الميسرين النفسيين والضيافة وأدوات الرسم والتنقل والمعرض الختامي، بإجمالي 4,000 دولار.", prompt: "افتح خريطة الميزانية.", page: 8, image: communityImage, caption: "الميزانية · موارد تصل إلى الميدان", tone: "warm" },
...
export default function Home() {
  ...
  const renderPage = (page: number) => {
    ...
    if (page === 4) return <article className="notebook-page page-campaign"><div className="page-number">05 <span>/ {total}</span></div><SectionKicker>المسار التدريبي</SectionKicker><div className="campaign-page-head"><div><h2>ست جلسات<br /><em>ثم يدخل البيت.</em></h2><p>مسار تطبيقي يدرّب الأب على التهدئة والوالدية الإيجابية والحماية دون عنف، ثم تنضم الأسرة في ورش مشتركة.</p></div><div className="campaign-card-note"><CalendarDays size={20} /><strong>06</strong><span>جلسات حضورية<br />في مجموعتين</span></div></div><div className="episode-book-grid">{workshops.map(([number, title, type]) => <button className="episode-book-card" key={number} onClick={() => setModal("guide")}><span>{number}</span><i><Check size={11} /></i><strong>{title}</strong><small>{type}</small></button>)}</div><div className="family-track"><div className="family-track-head"><UsersRound size={17} /><strong>أنشطة أسرية مشتركة</strong><small>يشارك فيها الأبناء والمراهقون</small></div><div className="family-track-list">{familyActivities.map(([title, detail, meta, Icon]) => <div key={title}><span><Icon size={16} /></span><div><strong>{title}</strong><p>{detail}</p><small>{meta}</small></div></div>)}</div></div><div className="page-bottom-note"><span>30 أباً</span><p>ينتج كل مشارك خطة أسرية قابلة للتطبيق، وتُراجع في الجلسات اللاحقة مع احترام الخصوصية.</p></div></article>;
    ...
    if (page === 6) return <article className="notebook-page page-practice"><div className="page-number">07 <span>/ {total}</span></div><SectionKicker>الأثر والمخرجات</SectionKicker><div className="practice-budget-grid"><div><h2>أرقام<br /><em>تخدم الناس.</em></h2><div className="practice-book-list">{impactStats.map(([value, suffix, label, note, Icon]) => <div key={label}><span><Icon size={15} /></span><div><strong>{value}{suffix} · {label}</strong><p>{note}</p></div><Check size={15} /></div>)}</div></div><div className="budget-book"><div className="budget-title"><span>توزيع المشاركين</span><strong>30</strong><small>أباً نازحاً في مجموعتين</small></div>{impactBreakdown.map((item) => <button className="budget-book-row" key={item.id} onClick={() => setImpactFocus(item.id)}><div><strong>{item.label}</strong><b>{item.percent}%</b></div><div className="budget-meter"><span className={impactFocus === item.id ? "budget-orange" : "budget-sage"} style={{ width: `${item.percent}%` }} /></div><small>{item.detail}</small></button>)}<div className="small-text-action"><ClipboardCheck size={14} /> قياس قبلي وبعدي مع تقرير تعلم</div></div></div><div className="outputs-block"><div className="outputs-head"><Target size={17} /><strong>مخرجات المبادرة</strong><small>ما يبقى بعد انتهاء الأسابيع الثمانية</small></div><div className="outputs-list">{outputs.map(([number, title, detail, Icon]) => <div key={number}><b>{number}</b><span><Icon size={15} /></span><div><strong>{title}</strong><p>{detail}</p></div></div>)}</div></div></article>;
    ...
    if (page === 8) return <article className="notebook-page page-budget"><div className="page-number">09 <span>/ {total}</span></div><SectionKicker>الميزانية</SectionKicker><div className="page-heading-row"><h2>4,000 دولار<br /><em>تصل إلى الميدان.</em></h2><p>ستة بنود واضحة تربط كل مبلغ بما يحتاجه الأب والميسر والعائلة.</p></div><div className="budget-line-list">{budgets.map(([title, detail, amount]) => <div key={title}><span className="budget-line-dot" /><div><strong>{title}</strong><p>{detail}</p></div><small>{amount}</small></div>)}</div><div className="budget-total"><span>الإجمالي</span><strong>4,000$</strong></div><div className="page-bottom-note"><span>المساءلة</span><p>يقدم الشريك تقريراً مالياً وتقرير تعلم يوضح التنفيذ والمخرجات دون كشف بيانات المشاركين.</p></div></article>;
