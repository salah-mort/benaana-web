...
const logoUrl = "/manus-storage/logo_7c90ce14.png";
const heroImage = "/manus-storage/binna-cover-2026_56a14709.png";
const communityImage = "/manus-storage/community_1b547e2d.png";
const mentorshipImage = "/manus-storage/mentorship_a47a42fe.png";
...
const storyScenes = [
  ...
  { number: "03", label: "المشهد الثالث", title: "فجوة بين الرغبة والأداة", intro: "الفجوة ليست في الرغبة بحماية الأسرة؛ بل في غياب مساحة آمنة وتدريب عملي ومتابعة تساعد الأب على تحويل النية إلى سلوك يومي قابل للملاحظة.", prompt: "سمِّ الفجوة التي تمنع التغيير.", page: 2, image: mentorshipImage, caption: "الفجوة · نية بلا أدوات", tone: "warm" },
  { number: "04", label: "المشهد الرابع", title: "حل ميداني يبدأ من الأب", intro: "بيننا تدخل وجاهي لا محاضرة: مجموعات صغيرة، تيسير يحترم الواقع، وأدوات عملية تنتقل من الجلسة إلى البيت دون إضافة عبء جديد.", prompt: "شاهد كيف يتحول الاحتياج إلى حل.", page: 3, image: communityImage, caption: "الحل · مساحات آمنة وتدريب تطبيقي", tone: "sage" },
  { number: "05", label: "المشهد الخامس", title: "ست جلسات من الفهم إلى التثبيت", intro: "يتدرج المسار من فهم الضغط والتهدئة إلى الإصغاء وتقاسم الرعاية والحزم دون عنف ثم خطة بيت أكثر أماناً.", prompt: "افتح المسار التدريبي.", page: 4, image: heroImage, caption: "التدريب · ست جلسات حضورية", tone: "terracotta" },
  { number: "06", label: "المشهد السادس", title: "إذا تغيّر الأب، اتسع الأمان", intro: "نربط بين الجلسة والممارسة والمتابعة: أب يلاحظ، يهدأ، يقترب، ثم ينقل أداة نافعة إلى أب آخر. هكذا تصبح النتيجة سلسلة لا لحظة.", prompt: "تابع منطق التغيير خطوة بخطوة.", page: 5, image: communityImage, caption: "منطق التغيير · ممارسة ثم متابعة", tone: "sand" },
  { number: "07", label: "المشهد السابع", title: "الأثر يُقاس باحترام", intro: "نقيس الوعي والممارسة قبلياً وبعدياً، ونوثق الخطط الأسرية والتعلم دون كشف خصوصية المشاركين. الأثر ليس رقماً منفصلاً عن حياة الناس.", prompt: "انظر إلى الأثر المتوقع بوضوح.", page: 6, image: mentorshipImage, caption: "الأثر · تعلم ومساءلة", tone: "sage" },
  ...
  { number: "11", label: "المشهد الأخير", title: "الشراكة هنا فعل حماية", intro: "دعم بيننا يعني تمويل جلسات آمنة، وتيسيراً مهنياً، وقياساً مسؤولاً، ومساحة لآباء يريدون أن يبنوا بيتاً أكثر أماناً رغم الحرب.", prompt: "افتح باب الشراكة.", page: 10, image: communityImage, caption: "الشراكة · لنَبْنِ بيننا", tone: "closing" },
];
...
export default function Home() {
  ...
  const renderPage = (page: number) => {
    ...
    if (page === 2) return <article className="notebook-page page-gap"><div className="page-number">03 <span>/ {total}</span></div><SectionKicker>الفجوة</SectionKicker><div className="idea-grid"><div><h2>نية طيبة<br /><em>بلا أدوات.</em></h2><p className="page-lead">الفجوة ليست في الرغبة بحماية الأسرة؛ بل في غياب مساحة آمنة وتدريب عملي ومتابعة تساعد الأب على تحويل النية إلى سلوك يومي قابل للملاحظة.</p></div><div className="idea-side"><div className="page-photo"><img src={communityImage} alt="مساحة حوار مجتمعية" /><span>من النية إلى الممارسة</span></div><blockquote><Quote size={19} /><p>بيننا تصل إلى المسافة بين ما يريد الأب فعله وما يستطيع فعله تحت الضغط.</p></blockquote></div></div><div className="page-bottom-note"><span>الفجوة</span><p>قلة التدخلات التي تخاطب الأب مباشرة كشريك في الأمان الأسري وتجمع التدريب والمتابعة والإحالة في مسار واحد.</p></div></article>;
    ...
    return <article className="notebook-page page-join"><div className="page-number">11 <span>/ {total}</span></div><div className="join-page-layout"><div><SectionKicker>الشراكة</SectionKicker><h2>لنَبْنِ<br /><em>بيننا.</em></h2><p>تحتاج المبادرة إلى شريك يؤمن بأن حماية الأسرة تبدأ من دعم الأب بأدوات عملية، وميسرين مؤهلين، ومساحة آمنة، وقياس مسؤول للأثر.</p><div className="join-page-buttons"><button className="notebook-primary" onClick={() => setModal("story")}>تواصل للشراكة <Send size={16} /></button><button className="notebook-quiet" onClick={() => setModal("files")}><FileText size={16} /> مكتبة الوثائق</button></div></div><div className="join-page-visual"><img src={communityImage} alt="دائرة حوار داعمة بين آباء وأفراد من المجتمع" /><div><HeartHandshake size={16} />الأمان يبدأ بيننا</div></div></div><div className="steps-book-row">{steps.map(([number, title, text, tag]) => <div key={number}><span>{number} · {tag}</span><strong>{title}</strong><p>{text}</p></div>)}</div><div className="join-signoff">أب واحد مدعوم، بيت أكثر أماناً <HeartHandshake size={16} /></div></article>;
  };
